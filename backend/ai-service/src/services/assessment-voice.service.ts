...
import {
  voiceEvaluationResponseSchema,
  type VoiceEvaluationRequest,
  type VoiceEvaluationResponse,
} from "../schemas/assessment-voice-evaluation.schema.js";
import {
  buildVoiceEvaluationPrompt,
  voiceEvaluationJsonSchema,
} from "../prompts/assessment-voice.prompt.js";
...
export async function evaluateVoiceTranscript(
  payload: VoiceEvaluationRequest
): Promise<VoiceEvaluationResponse> {
  const prompt = buildVoiceEvaluationPrompt(payload);
  ...
  const response = await client.responses.create({
    ...
    response_format: {
      type: "json_schema",
      json_schema: voiceEvaluationJsonSchema,
    },
    seed: hashSeed(payload.seed),
  });

  const jsonText = extractJsonText(response);
  const parsed = voiceEvaluationResponseSchema.parse(JSON.parse(jsonText));
  return parsed;
}
...
function extractJsonText(response: unknown): string {
  ...
  if ("toReadableStream" in response) {
    throw new Error(
      "Streaming responses are not supported for voice evaluation"
    );
  }
  ...
  logger.error(
    { response: structured },
    "AI voice evaluation response missing text"
  );
  throw new Error("AI service returned unexpected response format");
}
